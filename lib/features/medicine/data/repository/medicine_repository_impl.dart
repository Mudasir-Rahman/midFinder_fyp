...
class MedicineRepositoryImpl implements MedicineRepository {
  ...
  Future<Either<Failure, List<MedicineEntity>>> getAllMedicines() async {
    try {
      final medicines = await remoteDataSource.getAllMedicines();
      // ✅ MedicineModel extends MedicineEntity, so we can return directly
      return Right(medicines.cast<MedicineEntity>().toList());
    } on ServerFailure catch (e) {
      return Left(e);
    } catch (e) {
  ...
  Future<Either<Failure, void>> addMedicine(MedicineEntity medicine, {XFile? imageFile}) async {
    try {
      ...
      // ✅ Convert MedicineEntity to MedicineModel
      final medicineModel = MedicineModel(
        id: medicine.id,
        pharmacyId: medicine.pharmacyId,
        medicineName: medicine.medicineName,
  ...
  @override
  Future<Either<Failure, List<MedicineEntity>>> searchNearbyMedicine(
      String query,
      double latitude,
      double longitude
      ) async {
    try {
      final medicines = await remoteDataSource.searchNearbyMedicine(query, latitude, longitude);
      // ✅ MedicineModel extends MedicineEntity, so we can return directly
      return Right(medicines.cast<MedicineEntity>().toList());
    } on ServerFailure catch (e) {
      return Left(e);
